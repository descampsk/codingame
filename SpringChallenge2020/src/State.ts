...
export let bullets: Record<string, Bullet> = {};
export let bigBullets: Bullet[] = [];
...
const computeBigBullets = () => {
  bigBullets = Object.values(bullets)
    .filter((bullet) => bullet.value === 10)
    .sort(
      (a, b) =>
        Point.distance(a.position, myPacs[0].position) -
        Point.distance(b.position, myPacs[0].position)
    );
};
