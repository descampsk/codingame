  ...
  get owner() {
    const hasMineRobot =
      this.blocks.findIndex(
        (block) => block.owner === 1 /* ME */ && block.units > 0
      ) >= 0;
    const hasOpponentRobot =
      this.blocks.findIndex(
        (block) => block.owner === 0 /* OPPONENT */ && block.units > 0
      ) >= 0;
    if (hasMineRobot && hasOpponentRobot) return -2 /* BOTH */;
    if (hasMineRobot) return 1 /* ME */;
    if (hasOpponentRobot) return 0 /* OPPONENT */;
    return -1 /* NONE */;
  }
  ...
  action() {
    ...
    if (possibleRecyclers.length) {
      ...
      do {
        ...
        recycler = possibleRecyclers.shift();
